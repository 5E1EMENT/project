...
echo $temp;
echo '</select>';
echo '<center><input type="submit" class="btn btn-xs btn-success btn_add" name="sel_field" value="Открыть скважины"/></center>';
...
//and field.namefield='".$namefield."'"
    ...
    $table = "<form action='chink.php' method='GET'> 
 		...
 		<thead>
 			<tr>
 		<th>Номер месторождения</th><th>Номер скважины</th><th>Х координата</th><th>Y координатаа</th><th>Z координата</th><th>Угол Альфа</th><th>Угол Бета</th><th>Глубина</th><th>Просмотреть</th><th>Убрать</th></tr>
		</thead>
		<tbody>
 		";
    while ($row = mysqli_fetch_array($result))
    ...
    echo $table, '<br>';
    echo $row['d'];
    echo "<input type='submit' class='btn btn-xs btn-success btn_add btn_add_hole' name='add1' id='btn_add_hole' value='Добавить новую модель'/>";
